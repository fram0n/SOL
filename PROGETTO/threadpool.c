...
static void *workerpool_thread(void *threadpool) {    
    ...
    if(pool->connected == pool->numthreads) {
        int r;
        if((r=pthread_cond_signal(&(pool->condConn))) != 0) {
                UNLOCK_RETURN(&(pool->lock), NULL);
                errno = r;
    ...
    for (;;) {
        ...
        while((pool->count == 0) && (!pool->exiting)) {
            pthread_cond_wait(&(pool->condEmpty), &(pool->lock));
	    }
        ...
        if(pool->taskonthefly == 0 && pool->count == 0) {  //se non ci sono più task in attesa 
            if((r=pthread_cond_signal(&(pool->condEnd))) != 0) {
                UNLOCK_RETURN(&(pool->lock), NULL);
                errno = r;
                fprintf(stderr, "signal\n");
                return NULL;
...
static int freePoolResources(threadpool_t *pool) {
    if(pool->threads) {
        ...
        pthread_cond_destroy(&(pool->condEnd));
        pthread_cond_destroy(&(pool->condConn));
    }
    free(pool);    
    return 0;
}
...
threadpool_t *createThreadPool(int numthreads, int pending_size, struct sockaddr_un sockAddr) {
    ...
    || (pthread_cond_init(&(pool->condEnd), NULL) != 0) || (pthread_cond_init(&(pool->condConn), NULL) != 0))  {
        free(pool->threads);
        free(pool->pending_queue);
        free(pool);
        return NULL;
...
int destroyThreadPool(threadpool_t *pool, int force) {    
    ...
    if (pthread_cond_broadcast(&(pool->condEmpty)) != 0) {
...
int addToThreadPool(threadpool_t *pool, void (*f)(void *), void *arg) {
    ...
    int r;
    if((r=pthread_cond_signal(&(pool->condEmpty))) != 0) {
      UNLOCK_RETURN(&(pool->lock),-1);
      errno = r;
      return -1;
    }
